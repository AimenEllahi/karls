...
import {acceleratedRaycast, computeBoundsTree, disposeBoundsTree, CONTAINED, INTERSECTED, NOT_INTERSECTED, MeshBVHVisualizer, } from "./src/index.js";
...
//Declare Variables
let exportedModelName, originalModel, scene, camera, renderer, material, controls, targetMesh, brushMesh, brushMaterial, brush, symmetryBrush, bvhHelper, lastTime, maxSize, mouse = new THREE.Vector2(), mouseType = -1, brushActive = false, normalZ = new THREE.Vector3(0,0,1), lastMouse = new THREE.Vector2(), mouseState = false, lastMouseState = false, lastCastPose = new THREE.Vector3(), rightClick = false, sculptToggle = false;
let paintColor = {
    r: 199,
    g: 0,
    b: 57,
...
modelUploadInput.addEventListener("change", function(event) {
    ...
    if (file) {
        ...
        if (fileName.endsWith(".obj") || fileName.endsWith(".stl")) {
            ...
            alert("Please select a valid 3D model file with .obj, or .stl extension.");
        }
    }
});
...
function loadModelUrl(fileName, url=fileName) {
    ...
    if (fileName.endsWith(".obj")) {
        ...
        loader.load(url, (loadedObject)=>{
            ...
            originalModel = prepareMesh(loadedObject.children[0].geometry, material);
...
function performStroke(point, brushObject, brushOnly=false, accumulatedFields={}) {
    const {accumulatedTriangles=new Set(), accumulatedIndices=new Set(), accumulatedTraversedNodeIndices=new Set(), } = accumulatedFields;
...
const init = ()=>{
    ...
    let parent_iframe = window.parent.document.querySelector("iframe")?.parentElement;
    ...
    if (parent_iframe) {
        ...
        camera = new THREE.PerspectiveCamera(75,parentRect.width / parentRect.height,0.1,1000);
    } else {
        camera = new THREE.PerspectiveCamera(75,window.innerWidth / window.innerHeight,0.1,1000);
    }
    // init matcaps
    let urls = matcapUrls.forEach(url=>matcaps[url.slice(0, url.lastIndexOf('.'))] = new THREE.TextureLoader().load('./public/matcaps/' + url))
    let menu = {}
    ...
    let str = ``
    matcapUrls.forEach(url=>str += `<option value="${url}" style="background-image:url('./public/matcaps/${url}')">${url}</option>\n`)
    ...
    window.addEventListener("pointermove", function(e) {
        if (parent_iframe) {
            ...
            mouse.y = -(e.clientY / parentRect.height) * (parentRect.width < 768 ? 2 : 2) + 1;
        } else {
            mouse.x = (e.clientX / window.innerWidth) * 2 - 1;
            mouse.y = -(e.clientY / window.innerHeight) * (window.innerWidth < 768 ? 2 : 2) + 1;
        }
...
const paint = ()=>{
    ...
    if (controls.active || !brushActive) {
        ...
        let bscale = .13;
        brushMesh.scale.setScalar(params.size * .13);
